...
const ChatList = ({
  ...
  return (
    <div className="bg-white overflow-y-scroll h-[73vh] px-3 rounded-md shadow-sm animate-fade-in">
      {querys &&
        auth.searchUser?.map((item, index) => (
          <div
            key={index}
            onClick={() => handleClickOnChatCard(item.id)}
            className="cursor-pointer hover:bg-gray-100 transition-colors duration-300 rounded-md py-2"
          >
